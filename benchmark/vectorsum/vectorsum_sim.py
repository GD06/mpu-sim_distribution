...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the simulation for vectorsum",
    ...
    parser.add_argument("--output_perf_file", default=None,
                        help="The ooutput file storing performance metrics")
    ...
    ptr_b = hardware.mem.allocate(args.num_blocks * 4)
    ptr_c = hardware.mem.allocate(4)
    ...
    total_cycles, sim_freq = hardware.run_simulation(
        kernel=opt_kernel, 
        kernel_args=[ptr_a, ptr_b, args.vector_length],
        grid_dim=(1, 1, args.num_blocks),
        block_dim=(1, 1, args.num_threads),
        block_schedule=mapping_dict, 
    )
    print(
        "First execution finished: {} cycles at {} MHz".format(
            total_cycles, sim_freq)
    )

    if args.num_blocks != 1:
        total_cycles_2, sim_freq = hardware.run_simulation(
            kernel=opt_kernel,
            kernel_args=[ptr_b, ptr_c, args.num_blocks],
            grid_dim=(1, 1, 1),
            block_dim=(1, 1, args.num_threads),
            block_schedule={0: 0},
        )
        print(
            "Second execution finished: {} cycles at {} MHz".format(
                total_cycles_2, sim_freq)
        )
        c_in_bytes = hardware.mem.get_value(ptr_c, 4)
    else:
        total_cycles_2 = 0
        c_in_bytes = hardware.mem.get_value(ptr_b, 4)

    total_cycles += total_cycles_2
    print(
        "Total execution time {} cycles at {} MHz".format(
            total_cycles, sim_freq)
    )
    ...
    c_in_float = struct.unpack("1f", c_in_bytes)
    c = np.array(c_in_float).astype(np.float32)

    ground_truth = float(np.sum(a))
    np.testing.assert_allclose(c, [ground_truth], rtol=1e-3)
